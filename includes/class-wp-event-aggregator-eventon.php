...
class WP_Event_Aggregator_EventON {
	...
	public function import_event( $centralize_array, $event_args ){
		...
		if ( ! is_wp_error( $inserted_event_id ) ) {
			...
			$address = isset( $centralize_array['location']['address_1'] ) ? $centralize_array['location']['address_1']: '' ;
			if(  !empty ( $centralize_array['location']['full_address'] ) && isset( $centralize_array['location']['full_address'] ) ){
				$address = isset( $centralize_array['location']['full_address'] ) ? $centralize_array['location']['full_address'] : '' ;
			}
			...
			if( !empty( $centralize_array['location']['name'] ) && isset( $centralize_array['location']['name'] ) ){
