...
class WP_Event_Aggregator_Eventbrite_API {
	...
	public function import_events( $event_data = array() ){
		...
		$options = wpea_get_import_options( 'eventbrite' );
        $using_standard_api     = isset( $options['using_standard_api'] ) ? $options['using_standard_api'] : '';
        ...
        if( $using_standard_api === 'yes' ){
			if( $event_data['import_by'] == 'organizer_id' ){
				$eventbrite_api_url = 'https://www.eventbrite.com/api/v3/organizers/' . $organizer_id . '/events/?status=live&expand=venue,ticket_availability,organizer,organizer.logo,category';
			}elseif( $event_data['import_by'] == 'collection_id' ){
				$eventbrite_api_url = 'https://www.eventbrite.com/api/v3/collections/' . $collection_id . '/events/public/?time_filter=current_future&expand=venue,ticket_availability,organizer,organizer.logo,category';
			}
		}
	...
	public function generate_centralize_array( $eventbrite_event ) {
		...
		$event_image  = isset( $eventbrite_event['logo']['original']['url'] ) ? urldecode( $eventbrite_event['logo']['original']['url'] ) : '';
	...
	public function wpea_get_collection_name_by_id( $collection_id ) {
		...
		$get_collection = wp_remote_get(
			'https://www.eventbrite.com/api/v3/collections/' . $collection_id,
			array(
				'headers' => array(
