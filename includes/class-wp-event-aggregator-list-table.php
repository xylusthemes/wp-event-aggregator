	...
	function get_scheduled_import_data( $origin = '' ) {
		...
		if ( $importdata_query->have_posts() ) {
			while ( $importdata_query->have_posts() ) {
				...
				$totalimport = $wpdb->get_var( $totalimport_query ); //phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
...
class WPEA_Shortcode_List_Table extends WP_List_Table {
    ...
    private function table_data() {
		...
		$data[] = array(
                    ...
                    'shortcode'     => '<p class="wpea_short_code">[wp_events ajaxpagi="yes" ]</p>',
                    'action'     	=> "<button class='wpea-btn-copy-shortcode button-primary'  data-value='[wp_events ajaxpagi=\"yes\"]'>Copy</button>",
                    );
		$data[] = array(
					'id'            => 3,
